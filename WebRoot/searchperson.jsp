            ...
            <div class="pager-header">
                ...
                <div class="header-nav">
                    <a type="button" class="clickbutton"
                       href="/classCheck/serachemp?page=1&status=${status}&employeename=${employeename}&username=${username}">首页</a>
                    <c:choose>
                        <c:when test="${page>1}">
                            <a type="button" class="clickbutton"
                               href="/classCheck/serachemp?page=${page-1}&status=${status}&employeename=${employeename}&username=${username}">上页</a>
                        </c:when>
                        <c:otherwise>
                            上页
                        </c:otherwise>
                    ...
                    <c:choose>
                        <c:when test="${page<totalPage}">
                            <a type="button" class="clickbutton"
                               href="/classCheck/serachemp?page=${page+1}&status=${status}&employeename=${employeename}&username=${username}">下页</a>
                        </c:when>
                        <c:otherwise>
                            下页
                        </c:otherwise>
                    ...
                    <a type="button" class="clickbutton"
                       href="/classCheck/serachemp?page=${totalPage}&status=${status}&employeename=${employeename}&username=${username}">末页</a>
                    <form action="/classCheck/serachemp" method="post" style="display: inline">
        ...
        <table class="listtable">
            ...
            <c:forEach items="${list}" var="emp">
                <tr>
                    ...
                    <td>${emp.email}</td>
                    <td>
                        <form method="post" action="/classCheck/serachemp">
                            <input type="hidden" name="employeename" value="${employeename}">
                            <input type="hidden" name="username" value="${username}">
                            <input type="hidden" name="status" value="${status}">
